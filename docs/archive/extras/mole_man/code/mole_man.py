...
- Upper spinner builds the Mole Man Jackpot.
...
- Bigger Jackpot: Mole Man Jackpot is worth 2x.
...
class MoleMan(CaseFileMixin, Mode):
    MODE_KEY = "mole_man"
    DISPLAY_NAME = "Mole Man"
    ...
    def mode_start(self, **kwargs):
        ...
        self.publish_active_case_file_helpers([
            ...
            ("bigger_jackpots", "MOLE MAN JACKPOT 2X"),
        ...
        self.add_mode_event_handler("mole_man_complete_request", self._complete_mode)
        self.add_mode_event_handler("mole_man_fail_request", self._fail_mode)
    ...
    def _init_player_vars(self):
        ...
        player["mole_man_completed"] = 0
        player["mole_man_mode_points"] = 0
        player["mole_man_upper_target_hits"] = 0
        player["mole_man_spinner_hits"] = 0
        player["mole_man_jackpots"] = 0
        player["mole_man_best_jackpot"] = 0
        player["mole_man_jackpot_value"] = self._current_jackpot_value()
        player["mole_man_hurryup_seconds"] = 0
        player["mole_man_secret_target"] = ""
        player["mole_man_phase"] = self.phase
        player["mole_man_remaining_targets"] = 0
        player["mole_man_secret_revealed"] = 0
    ...
    def _upper_target_hit(self, **kwargs):
        ...
        if self.phase == "get_to_upper" or self.phase == "return_to_upper":
            self.machine.events.post("mole_man_upper_target_needs_new_search")
            return
        ...
        if non_secret_targets:
            ...
            self.machine.events.post(
                "mole_man_non_secret_target_dropped",
        ...
        self.machine.events.post(
            "mole_man_upper_target_reveal_hit",
    ...
    def _award_secret_jackpot(self, source="drop_target"):
        ...
        self._score(jackpot)
        self.machine.events.post(
            "mole_man_secret_jackpot_awarded",
            source=source,
    ...
    def _complete_mode(self, **kwargs):
        ...
        self.mode_done = True
        player = self.machine.game.player
        player["mole_man_completed"] = 1
        player["mole_man_phase"] = "complete"
        self.machine.events.post("mole_man_mode_complete")
    ...
    def _fail_mode(self, **kwargs):
        ...
        player = self.machine.game.player
        player["mole_man_completed"] = 0
        player["mole_man_phase"] = "failed"
        self.machine.events.post("mole_man_mode_failed")
    ...
    def _sync_vars(self):
        player = self.machine.game.player
        player["mole_man_mode_points"] = self.mode_points
        player["mole_man_upper_target_hits"] = self.upper_target_hits
        player["mole_man_spinner_hits"] = self.spinner_hits
        player["mole_man_jackpots"] = self.jackpots_collected
        player["mole_man_best_jackpot"] = self.best_jackpot
        player["mole_man_jackpot_value"] = self._current_jackpot_value()
        player["mole_man_hurryup_seconds"] = self.hurryup_seconds_left
        player["mole_man_secret_target"] = self.TARGET_LABELS[self.secret_target] if self.secret_target else ""
        player["mole_man_phase"] = self.phase
        player["mole_man_remaining_targets"] = len(self.standing_targets)
        player["mole_man_secret_revealed"] = 1 if self.phase == "hurryup" else 0
