...
class Electro(CaseFileMixin, Mode):
    ...
    def pick_next_lit_shot(self):
        ...
        # Publish the ordinary spark status first; the countdown must be the
        # final status event so its large red timer remains visible.
        self._update_status()
        ...
        if previous_location != "upper" and self.current_shot.group == "upper":
            ...
            self.machine.events.post("electro_vuk_chase_start")
    ...
    def start_super_jackpot(self, shot):
        ...
        # As above, let the countdown replace the ordinary spark status rather
        # than immediately replacing the timer after it is shown.
        self._update_status()
        self._show_message(
            "SUPER SURGE LIT",
