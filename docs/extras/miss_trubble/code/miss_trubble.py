...
class MissTrubble(CaseFileMixin, Mode):
    BASE_ROUNDS = 7
    EXTRA_ROUNDS = 2
    ...
    def mode_start(self, **kwargs):
        ...
        self.publish_case_file_bonus_events("miss_trubble")
        ...
        for group in self.SHOT_GROUPS:
            self.add_mode_event_handler(f"miss_trubble_{group}_hit", self._shot_hit, group=group)

        self.add_mode_event_handler("miss_trubble_spinner_reveal", self._reveal_current_round)
        self.add_mode_event_handler("miss_trubble_spinner_switch", self._spinner_switch)
        self.add_mode_event_handler("miss_trubble_complete_request", self._complete_mode)
        self.add_mode_event_handler("miss_trubble_fail_request", self._fail_mode)

        self._sync_player_vars()
        self.machine.events.post("miss_trubble_startup_complete")
        self.machine.events.post("show_mode_message_long", message_mode_title="MISS TRUBBLE", message_mode_subtitle="FIND THE CORRECT SHOT")
        self._start_next_round()

    def mode_stop(self, **kwargs):
        self.delay.remove("miss_trubble_hide_reveal")
    ...
    def _reveal_current_round(self, **kwargs):
        ...
        for group in self.current_groups:
            if all_good or group == self.correct_group:
                self.machine.events.post(f"miss_trubble_reveal_good_{group}")
            else:
                self.machine.events.post(f"miss_trubble_reveal_bad_{group}")

        self.delay.remove("miss_trubble_hide_reveal")
        self.delay.add(
            name="miss_trubble_hide_reveal",
            ms=self.reveal_ms,
    ...
    def _show_hidden_round(self, **kwargs):
        ...
        self.machine.events.post("miss_trubble_all_lights_off")
        for group in self.current_groups:
            self.machine.events.post(f"miss_trubble_hidden_{group}")
    ...
    def _complete_mode(self, **kwargs):
        ...
        self.machine.events.post("miss_trubble_all_lights_off")
        self.machine.events.post("show_mode_message_long", message_mode_title="MISS TRUBBLE DEFEATED", message_mode_subtitle="SEQUENCE COMPLETE")
    ...
    def _fail_mode(self, **kwargs):
        ...
        self.machine.events.post("miss_trubble_all_lights_off")
        self.machine.events.post("show_mode_message_long", message_mode_title="MISS TRUBBLE ESCAPES", message_mode_subtitle="TOO MUCH TROUBLE")
    ...
    def _sync_player_vars(self):
        ...
        if not player:
            return
        player["miss_trubble_mode_points"] = self.mode_points
        player["miss_trubble_round"] = self.round_number
        player["miss_trubble_total_rounds"] = self.rounds_to_play
        player["miss_trubble_correct_shots"] = self.correct_shots
        player["miss_trubble_incorrect_shots"] = self.incorrect_shots
        player["miss_trubble_next_jackpot"] = self.jackpot_value
