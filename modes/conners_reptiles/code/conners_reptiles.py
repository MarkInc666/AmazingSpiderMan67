...
class ConnersReptiles(CaseFileMixin, Mode):
    ...
    def mode_start(self, **kwargs):
        ...
        self.publish_case_file_bonus_events("conners_reptiles")
        self.publish_active_case_file_helpers([
            ("more_jackpots", "CENTER WEB RAMPAGE JACKPOT ADDED"),
            ("bigger_jackpots", "CONNER'S REPTILES JACKPOTS 2X"),
        ...
        player["conners_reptiles_state"] = 2
        self._sync_vars()

        self.add_mode_event_handler("conners_reptiles_pop_hit", self._pop_hit)
        for target in [1, 2, 3]:
            self.add_mode_event_handler(
                f"conners_reptiles_left_drop_{target}_hit",
                self._left_drop_hit,
                target=target,
            )
        ...
        for target in [1, 2, 3, 4, 5]:
            self.add_mode_event_handler(
                f"conners_reptiles_right_drop_{target}_hit",
                self._right_drop_hit,
                target=target,
            )
        self.add_mode_event_handler("conners_reptiles_upper_middle_hit", self._upper_middle_hit)
        self.add_mode_event_handler("conners_reptiles_center_web_hit", self._center_web_hit)
        self.add_mode_event_handler("conners_reptiles_saucer_2_hit", self._saucer_2_hit)
        self.add_mode_event_handler("conners_reptiles_fail_request", self._complete_mode)

        if self.has_case_file("safety_net"):
            self.machine.events.post("start_case_file_ball_save")

        self.machine.events.post("conners_reptiles_startup_complete")
    ...
    def mode_stop(self, **kwargs):
        ...
        self.delay.remove("conners_reptiles_super_timer")
    ...
    def _light_rampage_shot(self, shot):
        ...
        self.machine.events.post("conners_reptiles_rampage_shot_lit", shot=shot)
    ...
    def _stage_left_bank(self):
        ...
        if not self.shot_assist_active:
            self.delay.add(
                ...
                name="conners_reptiles_stage_left_bank",
            )
    ...
    def _stage_right_bank(self):
        ...
        self.delay.add(
            ...
            name="conners_reptiles_stage_right_bank",
        )
    ...
    def _light_super_jackpot(self):
        ...
        self.super_lit = True
        self._close_rooftop_gate()
        self.machine.events.post("conners_reptiles_clear_rampage_lights")
    ...
    def _restart_super_timer(self):
        ...
        self.delay.add(
            ...
            name="conners_reptiles_super_timer",
        )
        self.machine.events.post("conners_reptiles_super_timer_started", seconds=seconds)
    ...
    def _super_timer_expired(self):
        if self._in_summary_or_done():
            return
        self.machine.events.post("conners_reptiles_super_timer_expired")
        self._show_mode_message("CONNER'S REPTILES ESCAPED", "SUPER TIMER EXPIRED")
        self._fail_mode()
    ...
    def _saucer_2_hit(self, **kwargs):
        if self._in_summary_or_done():
            return
        if not self.super_lit:
            self.machine.events.post("conners_reptiles_saucer_unlit")
            return
        ...
        self.machine.events.post(
            "conners_reptiles_super_jackpot_collected",
            value=self.super_jackpot_value,
        )
        self._show_mode_jackpot("CONNER'S REPTILES SUPER JACKPOT", self.super_jackpot_value)
        self._complete_mode()
    ...
    def _open_rooftop_gate(self):
        ...
        self.machine.events.post("conners_reptiles_rooftop_gate_opened")
    ...
    def _close_rooftop_gate(self):
        ...
        self.machine.events.post("conners_reptiles_rooftop_gate_closed")

    def _sync_vars(self):
        player = self.machine.game.player
        player["active_mode_points"] = self.mode_points
        player["conners_reptiles_pop_hits"] = self.pop_hits
        player["conners_reptiles_rampage_level"] = self.rampage_level
        player["conners_reptiles_current_jackpot"] = self._display_jackpot_value()
        player["conners_reptiles_super_jackpot"] = self.super_jackpot_value
        ...
        player["conners_reptiles_pop_score_value"] = self.pop_score_value
        player["conners_reptiles_super_lit"] = 1 if self.super_lit else 0
        player["conners_reptiles_super_timer_seconds"] = self._super_timer_seconds()
