...
class FlyBrothers(CaseFileMixin, Mode):
    """FLY BROTHERS - STAN AND LEE PATTERSON
    ...
    MODE_KEY = "fly_brothers"
    DISPLAY_NAME = "Fly Brothers"
    ...
    def mode_start(self, **kwargs):
        ...
        if self.has_case_file("safety_net"):
            ...
            self.machine.events.post("fly_brothers_safety_net_lit")

        self.machine.events.post("clear_saucers_delayed")
        self.machine.events.post("drop_target_bank_dt_bank_left_reset")
        self.machine.events.post("drop_target_bank_dt_bank_right_reset")
        self.machine.events.post("fly_brothers_clear_all_lights")
        self.machine.events.post("fly_brothers_round_started", round=self.round_number)
        self.machine.events.post("fly_brothers_upper_access_needed")
        self.machine.events.post("fly_brothers_start_more_time_multiball" if self.has_case_file("more_time") else "fly_brothers_start_multiball")
        self._sync_vars()
        self._show_mode_message("FLY BROTHERS", "GET TO THE ROOF")
    ...
    def _reset_player_vars(self):
        ...
        player["fly_brothers_state"] = 1
        player["active_mode_points"] = 0
        player["active_mode_hits"] = 0
        player["active_mode_major_hits"] = 0
        player["fly_brothers_round"] = self.round_number
        player["fly_brothers_rounds_completed"] = 0
        player["fly_brothers_captures"] = 0
        player["fly_brothers_super_jackpots"] = 0
        player["fly_brothers_jackpots"] = 0
        player["fly_brothers_spinner_hits"] = 0
        player["fly_brothers_base_jackpot"] = self.base_jackpot
        player["fly_brothers_lit_saucers"] = 0
        player["fly_brothers_multiplier"] = 0
        player["fly_brothers_last_award"] = 0
        player["fly_brothers_shot_assist_used"] = 0

        player["fly_brothers_upper_left_hit"] = 0
        player["fly_brothers_upper_center_hit"] = 0
        player["fly_brothers_upper_right_hit"] = 0
        player["fly_brothers_saucer_1_lit"] = 0
        player["fly_brothers_saucer_2_lit"] = 0
        player["fly_brothers_saucer_3_lit"] = 0

    def _add_handlers(self):
        self.add_mode_event_handler("fly_brothers_multiball_ended", self._multiball_ended)
        self.add_mode_event_handler("fly_brothers_complete_request", self._complete_mode)
        self.add_mode_event_handler("fly_brothers_fail_request", self._complete_mode)
        self.add_mode_event_handler("fly_brothers_upper_spinner_hit", self._upper_spinner_hit)
        self.add_mode_event_handler("fly_brothers_upper_target_left_hit", self._upper_target_hit, target="left")
        self.add_mode_event_handler("fly_brothers_upper_target_center_hit", self._upper_target_hit, target="center")
        self.add_mode_event_handler("fly_brothers_upper_target_right_hit", self._upper_target_hit, target="right")
        self.add_mode_event_handler("fly_brothers_saucer_1_hit", self._saucer_hit, saucer="saucer_1")
        self.add_mode_event_handler("fly_brothers_saucer_2_hit", self._saucer_hit, saucer="saucer_2")
        self.add_mode_event_handler("fly_brothers_saucer_3_hit", self._saucer_hit, saucer="saucer_3")
    ...
    def _upper_spinner_hit(self, **kwargs):
        ...
        self.spinner_hits += 1
        self._sync_vars()
        self.machine.events.post("fly_brothers_jackpot_built", value=self._current_base_jackpot(), spinner_hits=self.spinner_hits)
    ...
    def _upper_target_hit(self, target, **kwargs):
        ...
        if self.has_case_file("shot_assist") and not self.shot_assist_used:
            ...
            self.machine.events.post("fly_brothers_shot_assist_used")
            self.machine.events.post("fly_brothers_upper_access_complete")
        ...
        self.machine.events.post(
            "fly_brothers_upper_target_awarded",
            target=target,
    ...
    def _light_saucer_for_target(self, target):
        ...
        self.machine.events.post(f"fly_brothers_upper_{target}_complete")
        self.machine.events.post(f"fly_brothers_{saucer}_lit")
    ...
    def _saucer_hit(self, saucer, **kwargs):
        ...
        if saucer not in self.lit_saucers:
            ...
            self.machine.events.post("fly_brothers_unlit_saucer_hit", saucer=saucer)
        ...
        if self.round_number == self.SUPER_ROUND:
            ...
            self._score(award)
            self.machine.events.post("fly_brothers_super_jackpot_collected", value=award)
            self._show_mode_jackpot("SUPER JACKPOT", award, "THE FLIES ESCAPE")
            self._reset_cycle_to_brother_1()
        else:
            self.captures += 1
            self._score(award)
            self.machine.events.post("fly_brothers_brother_captured", brother=self.round_number, value=award)
    ...
    def _reset_round_progress(self):
        ...
        self.machine.events.post("fly_brothers_round_reset")
        self.machine.events.post("fly_brothers_round_started", round=self.round_number)
        self.machine.events.post("fly_brothers_upper_access_needed")
        self._sync_vars()
    ...
    def _complete_mode(self, **kwargs):
        ...
        self.mode_done = True
        player = self.machine.game.player
        player["fly_brothers_state"] = 2
        self.machine.events.post("fly_brothers_mode_complete")
    ...
    def _sync_vars(self):
        ...
        player["fly_brothers_round"] = self.round_number
        player["fly_brothers_rounds_completed"] = self.rounds_completed
        player["fly_brothers_captures"] = self.captures
        player["fly_brothers_super_jackpots"] = self.super_jackpots
        player["fly_brothers_jackpots"] = self.jackpots_collected
        player["fly_brothers_spinner_hits"] = self.spinner_hits
        player["fly_brothers_base_jackpot"] = self._current_base_jackpot()
        player["fly_brothers_lit_saucers"] = len(self.lit_saucers)
        player["fly_brothers_multiplier"] = self._current_multiplier()
        player["fly_brothers_last_award"] = self.last_award
        player["fly_brothers_shot_assist_used"] = 1 if self.shot_assist_used else 0

        player["fly_brothers_upper_left_hit"] = 1 if "left" in self.upper_targets_hit else 0
        player["fly_brothers_upper_center_hit"] = 1 if "center" in self.upper_targets_hit else 0
        player["fly_brothers_upper_right_hit"] = 1 if "right" in self.upper_targets_hit else 0
        player["fly_brothers_saucer_1_lit"] = 1 if "saucer_1" in self.lit_saucers else 0
        player["fly_brothers_saucer_2_lit"] = 1 if "saucer_2" in self.lit_saucers else 0
        player["fly_brothers_saucer_3_lit"] = 1 if "saucer_3" in self.lit_saucers else 0
