...
class FlyTwins(CaseFileMixin, Mode):
    """THE FLY TWINS - STAN AND LEE PATTERSON
    ...
    def mode_start(self, **kwargs):
        ...
        if self.has_case_file("safety_net"):
            ...
            self.machine.events.post("fly_twins_safety_net_lit")
        ...
        self.machine.events.post("fly_twins_round_started", round=self.round_number)
    ...
    def _add_handlers(self):
        ...
        self.add_mode_event_handler("fly_twins_complete_request", self._complete_mode)
        self.add_mode_event_handler("fly_twins_fail_request", self._complete_mode)
        self.add_mode_event_handler("fly_twins_upper_spinner_hit", self._upper_spinner_hit)
        self.add_mode_event_handler("fly_twins_upper_target_left_hit", self._upper_target_hit, target="left")
        self.add_mode_event_handler("fly_twins_upper_target_center_hit", self._upper_target_hit, target="center")
        self.add_mode_event_handler("fly_twins_upper_target_right_hit", self._upper_target_hit, target="right")
        self.add_mode_event_handler("fly_twins_saucer_1_hit", self._saucer_hit, saucer="saucer_1")
        self.add_mode_event_handler("fly_twins_saucer_2_hit", self._saucer_hit, saucer="saucer_2")
        self.add_mode_event_handler("fly_twins_saucer_3_hit", self._saucer_hit, saucer="saucer_3")
    ...
    def _upper_spinner_hit(self, **kwargs):
        ...
        self.spinner_hits += 1
        self._sync_vars()
        self.machine.events.post("fly_twins_jackpot_built", value=self._current_base_jackpot(), spinner_hits=self.spinner_hits)
    ...
    def _upper_target_hit(self, target, **kwargs):
        ...
        if self.has_case_file("shot_assist") and not self.shot_assist_used:
            ...
            self.machine.events.post("fly_twins_shot_assist_used")
            self.machine.events.post("fly_twins_upper_access_complete")
        ...
        self.machine.events.post(
            "fly_twins_upper_target_awarded",
            target=target,
    ...
    def _light_saucer_for_target(self, target):
        ...
        self.machine.events.post(f"fly_twins_upper_{target}_complete")
        self.machine.events.post(f"fly_twins_{saucer}_lit")
    ...
    def _saucer_hit(self, saucer, **kwargs):
        ...
        if saucer not in self.lit_saucers:
            ...
            self.machine.events.post("fly_twins_unlit_saucer_hit", saucer=saucer)
        ...
        if self.round_number == self.SUPER_ROUND:
            ...
            self._score(award)
            self.machine.events.post("fly_twins_super_jackpot_collected", value=award)
    ...
    def _reset_round_progress(self):
        ...
        self.machine.events.post("fly_twins_round_reset")
        self.machine.events.post("fly_twins_round_started", round=self.round_number)
        self.machine.events.post("fly_twins_upper_access_needed")
        self._sync_vars()
    ...
    def _complete_mode(self, **kwargs):
        ...
        self.mode_done = True
        player = self.machine.game.player
        player["fly_twins_state"] = 2
        self.machine.events.post("fly_twins_mode_complete")
