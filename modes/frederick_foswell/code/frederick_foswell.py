...
class FrederickFoswell(Mode, CaseFileMixin):
    """Frederick Foswell — Front Page Frame-Up.

    Pops build Rumours. Spinner converts 2 Rumours into Headlines at saucers.
    Collect 3 Headlines to light Daily Bugle VUK for Super. After each Super,
    Rumours start draining; pops can keep adding Rumours and the loop repeats
    until the Rumours run out.
    """

    MODE_KEY = "frederick_foswell"
    DISPLAY_NAME = "Frederick Foswell"
    ...
    def mode_start(self, **kwargs):
        ...
        self.add_mode_event_handler("frederick_foswell_pop_hit", self._pop_hit)
        self.add_mode_event_handler("frederick_foswell_spinner_hit", self._spinner_hit)
        self.add_mode_event_handler("frederick_foswell_saucer_1_hit", self._saucer_hit, saucer=1)
        self.add_mode_event_handler("frederick_foswell_saucer_2_hit", self._saucer_hit, saucer=2)
        self.add_mode_event_handler("frederick_foswell_saucer_3_hit", self._saucer_hit, saucer=3)
        self.add_mode_event_handler("frederick_foswell_vuk_hit", self._vuk_hit)
        self.add_mode_event_handler("frederick_foswell_back_page_hit", self._back_page_hit)
        ...
        self.publish_case_file_bonus_events("frederick_foswell")
    ...
    def mode_stop(self, **kwargs):
        self.delay.remove("frederick_foswell_rumour_drain")
        self.delay.remove("frederick_foswell_back_page_timeout")
        self.clear_active_case_file_helpers()
        self.machine.events.post("frederick_foswell_clear_lights")
    ...
    def _spinner_hit(self, **kwargs):
        ...
        self.machine.events.post(
            "frederick_foswell_headline_lit",
            saucer=saucer,
    ...
    def _collect_headline(self, saucer, assisted=False):
        ...
        self.machine.events.post(
            "frederick_foswell_headline_collected",
            saucer=saucer,
            assisted=assisted,
    ...
    def _start_rumour_drain(self):
        ...
        self.delay.remove("frederick_foswell_rumour_drain")
        self.delay.add(
            name="frederick_foswell_rumour_drain",
            ms=self.drain_ms,
    ...
    def _start_back_page_bonus(self, super_value):
        ...
        self.delay.remove("frederick_foswell_back_page_timeout")
        self.delay.add(
            name="frederick_foswell_back_page_timeout",
            ms=10000,
    ...
    def _update_lights(self):
        ...
        for saucer in self.headlines_lit:
            self.machine.events.post(f"frederick_foswell_saucer_{saucer}_lit")

        if self.super_lit:
            self.machine.events.post("frederick_foswell_super_lit_show")

        if self.back_page_lit:
            self.machine.events.post("frederick_foswell_back_page_lit_show")
