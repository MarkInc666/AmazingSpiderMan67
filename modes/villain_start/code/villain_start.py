...
class VillainStart(Mode):
    ...
    def _villain_started(self, villain_key=None, villain_name=None, villain=None, **kwargs):
        key = villain_key or villain or villain_name or self.machine.game.player["villain_current_name"]
        self.villain_start_logic_active = False
        ...
        # Daily Bugle owns persistent keyed A/B shows at a higher mode priority.
        # Clear that owner first, then force its paused visual state off.
    ...
    def _villain_select_cancelled(self, **kwargs):
        if self.machine.game.player["villain_current_name"] == "villain_select":
            self._unlock_start_logic()

    def _villain_start_request_failed(self, **kwargs):
        if self.machine.game.player["villain_current_name"] in ("villain_select", "villain_start"):
            self._unlock_start_logic()
