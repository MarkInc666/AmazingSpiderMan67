...
class Fakir(CaseFileMixin, Mode):
    ...
    def mode_start(self, **kwargs):
        ...
        self.publish_active_case_file_helpers([
            ("more_jackpots", "TWO EXTRA ILLUSIONS"),
            ("bigger_jackpots", "BIGGER FAKIR JACKPOTS"),
        ...
        for group in self.SHOT_GROUPS:
            self.add_mode_event_handler(f"fakir_{group}_hit", self._shot_hit, group=group)

        self.add_mode_event_handler("fakir_spinner_reveal", self._reveal_current_round)
        self.add_mode_event_handler("fakir_spinner_switch", self._spinner_switch)
        self.add_mode_event_handler("fakir_complete_request", self._complete_mode)
        self.add_mode_event_handler("fakir_fail_request", self._complete_mode)

        self._sync_player_vars()
        self.machine.events.post("fakir_startup_complete")
        self.machine.events.post("show_mode_message_long", message_mode_title="FANTASTIC FAKIR", message_mode_subtitle="FIND THE TRUE SHOT")
        self._start_next_round()

    def mode_stop(self, **kwargs):
        self.delay.remove("fakir_hide_reveal")
    ...
    def _start_next_round(self):
        ...
        self.machine.events.post("show_mode_message", message_mode_title="FAKIR ROUND", message_mode_subtitle=f"{self.round_number} OF {self.rounds_to_play}", message_mode_value=self.jackpot_value)
    ...
    def _collect_jackpot(self, group):
        ...
        self.machine.events.post(f"fakir_correct_{group}")
        self.machine.events.post("show_mode_jackpot", message_mode_title="TRUE SHOT", message_mode_subtitle=group.replace("_", " ").upper(), message_mode_value=self.jackpot_value)
    ...
    def _collect_bad_shot(self, group):
        ...
        self.machine.events.post(
            "fakir_incorrect_shot",
            group=group,
            incorrect_shots=self.incorrect_shots,
            max_incorrect=self.MAX_INCORRECT_SHOTS,
        )
        self.machine.events.post(f"fakir_incorrect_{group}")
        self.machine.events.post("show_mode_message", message_mode_title="WRONG SHOT", message_mode_subtitle=f"ILLUSION {self.incorrect_shots} / {self.MAX_INCORRECT_SHOTS}")
        self._sync_player_vars()
    ...
    def _reveal_current_round(self, **kwargs):
        ...
        for group in self.current_groups:
            if all_good or group == self.correct_group:
                self.machine.events.post(f"fakir_reveal_good_{group}")
            else:
                self.machine.events.post(f"fakir_reveal_bad_{group}")

        self.delay.remove("fakir_hide_reveal")
        self.delay.add(
            name="fakir_hide_reveal",
            ms=self.reveal_ms,
    ...
    def _show_hidden_round(self, **kwargs):
        ...
        self.machine.events.post("fakir_all_lights_off")
        for group in self.current_groups:
            self.machine.events.post(f"fakir_hidden_{group}")
    ...
    def _complete_mode(self, **kwargs):
        ...
        if player:
            player["fakir_state"] = 2
        self.machine.events.post("fakir_all_lights_off")
        self.machine.events.post("show_mode_message_long", message_mode_title="FANTASTIC FAKIR DEFEATED", message_mode_subtitle="SEQUENCE COMPLETE")
    ...
    def _fail_mode(self, **kwargs):
        ...
        if player:
            player["fakir_state"] = 2
        self.machine.events.post("fakir_all_lights_off")
        self.machine.events.post("show_mode_message_long", message_mode_title="FANTASTIC FAKIR ESCAPES", message_mode_subtitle="TOO MUCH TROUBLE")
        self.machine.events.post("fakir_mode_complete")
    ...
    def _sync_player_vars(self):
        ...
        player["fakir_round"] = self.round_number
        player["fakir_total_rounds"] = self.rounds_to_play
        player["fakir_correct_shots"] = self.correct_shots
        player["fakir_incorrect_shots"] = self.incorrect_shots
        player["fakir_next_jackpot"] = self.jackpot_value
